...
function activeAt(day,min){return S.filter(s=>s.day===day&&s.startMin<=min&&min<s.endMin).sort((a,b)=>b.score-a.score||a.startMin-b.startMin||a.id-b.id)}
function recommendation(s){const active=activeAt(s.day,s.startMin);return {rank:active.findIndex(x=>x.id===s.id)+1,count:active.length,best:active[0]}}
function filtered(){const q=$('#search').value.trim().toLowerCase(), st=$('#stage').value, type=$('#type').value;return S.filter(s=>s.day===state.day&&(!st||s.stage===st)&&(!type||s.type===type)&&(!state.relevant||s.score>=65)&&(!state.suggested||s.route)&&(!state.myAgenda||store.selected.has(s.id))&&(!state.favorites||store.fav.has(s.id))&&(!q||[s.title,s.speakers,s.track,s.type,s.stage,D[s.id]?.summary||'',...(D[s.id]?.speakers||[]).map(p=>`${p.role} ${p.organization||""}`)].join(' ').toLowerCase().includes(q)))}
...
function renderBoard(F){
 ...
 stages.forEach((st,idx)=>{html+=`<div class="stagecol" style="grid-column:${idx+2};height:${H}px">`;for(let m=startBase;m<=endBase;m+=10)html+=`<div class="gridline ${m%30===0?'major':''}" style="top:${(m-startBase)*px}px"></div>`;F.filter(s=>s.stage===st).forEach(s=>{const y=(s.startMin-startBase)*px,h=Math.max(26,(s.endMin-s.startMin)*px-3),r=recommendation(s),best=r.rank===1;html+=`<button class="session ${best?'recommended':''} ${s.route?'suggested':''} ${store.selected.has(s.id)?'selected':''} ${store.fav.has(s.id)?'favorite':''}" data-tier="${s.tier}" data-open="${s.id}" style="top:${y}px;height:${h}px"><div class="s-time">${s.start}–${s.end} · ${esc(s.type)}</div><div class="s-title">${esc(clean(s.title))}</div><div class="s-rank ${best?'best':''}">${best?'★ RECOMENDADA':`#${r.rank}/${r.count} a las ${s.start}`}</div></button>`});html+='</div>'});html+='</div>';$('#board').innerHTML=html;
}
function renderMobile(F){
 ...
 $('#mobileList').innerHTML=starts.map(t=>{const items=F.filter(s=>s.start===t).sort((a,b)=>b.score-a.score),first=items[0],best=activeAt(state.day,first.startMin)[0];return `<section class="time-group" data-time="${t}"><h3>${t}</h3><div class="time-recommendation">★ Mejor opción general a las ${t}: <strong>${esc(clean(best.title))}</strong> · ${esc(best.stage)}${F.some(x=>x.id===best.id)?'':' · fuera del filtro actual'}</div>${items.map(s=>{const r=recommendation(s),highlight=r.rank===1;return `<details class="mobile-card ${highlight?'recommended':''}"><summary><div class="m-top"><div class="badges"><span class="badge ${highlight?'recommended-badge':'rank'}">${highlight?'★ RECOMENDADA':`#${r.rank}/${r.count} activas`}</span><span class="badge">${esc(s.stage)}</span><span class="badge">${esc(s.type)}</span>${store.selected.has(s.id)?'<span class="badge sel">✓ Mi agenda</span>':''}</div><span>${store.fav.has(s.id)?'★':''}</span></div><div class="m-title">${esc(clean(s.title))}</div><div class="m-meta">${s.start}–${s.end} · ${esc(s.track||'Sin track')} · ${tier(s)}</div></summary><div class="m-body"><h4>Resumen de la descripción oficial</h4><p>${esc(D[s.id]?.summary||'Descripción no publicada por el organizador.')}</p><h4>Ponentes</h4>${speakerHtml(s)}<h4>Por qué te puede interesar</h4><p>${esc(s.why)}</p><button class="control" data-open="${s.id}">Ver conflictos y guardar</button></div></details>`}).join('')}</section>`}).join('');
}
function speakerHtml(s){const people=D[s.id]?.speakers||[];return people.length?`<div class="speaker-list">${people.map(p=>`<details class="speaker-card"><summary class="speaker">${p.photo?`<img class="speaker-photo" src="${esc(p.photo)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:`<div class="speaker-photo speaker-fallback">${esc(p.name.slice(0,1))}</div>`}<span class="speaker-copy"><strong>${esc(p.name)}</strong><span>${esc(p.role?(p.organization?`${p.role} · ${p.organization}`:p.role):(p.organization||'Perfil no publicado por el organizador.'))}</span></span><span class="speaker-chevron" aria-hidden="true">⌄</span></summary><div class="speaker-extra"><p>${esc(p.bio||'El organizador todavía no ha publicado una biografía de este ponente.')}</p>${(p.socials||[]).length?`<div class="speaker-socials">${p.socials.filter(a=>a.url?.startsWith('https://')).map(a=>`<a href="${esc(a.url)}" target="_blank" rel="noopener noreferrer">${esc(a.label)} ↗</a>`).join('')}</div>`:'<span class="no-socials">No hay redes publicadas en la ficha oficial.</span>'}</div></details>`).join('')}</div>`:'<p>El organizador no ha publicado ponentes para esta sesión.</p>'}
function openDrawer(id){
 ...
 $('#drawerContent').innerHTML=`<div class="drawer-inner"><button class="close" id="closeDrawer" aria-label="Cerrar">×</button><div class="overline">${s.start}–${s.end} · ${esc(s.stage)}</div><h2>${esc(s.title)}</h2><div class="badges"><span class="badge">${esc(s.type)}</span>${s.track?`<span class="badge">${esc(s.track)}</span>`:''}<span class="badge">${tier(s)}</span>${s.route?'<span class="badge">◇ Ruta sugerida</span>':''}</div><div class="rankbox ${r.rank===1?'rankbox-best':''}"><strong>${r.rank===1?'★ RECOMENDADA':`#${r.rank} de ${r.count}`}</strong><span> ${r.rank===1?`para las ${s.start}`:`entre las sesiones activas a las ${s.start}`}</span>${r.rank!==1?`<div class="rank-best-name">La recomendada: ${esc(clean(r.best.title))}</div>`:''}</div><h3>Resumen de la descripción oficial</h3><p>${esc(D[s.id]?.summary||'Descripción no publicada por el organizador.')}</p><h3>Ponentes</h3>${speakerHtml(s)}<h3>Por qué te puede interesar</h3><p>${esc(s.why)}</p><h3>Sesiones que compiten con esta</h3><div class="conflictbox">${cs.length?cs.slice(0,10).map((x,i)=>`<div>${i+1}. <b>${esc(clean(x.title))}</b><br><span>${x.start}–${x.end} · ${esc(x.stage)}</span></div>`).join('<br>'):'No hay solapes directos.'}</div><div class="actionrow"><button id="selectSession" class="${store.selected.has(s.id)?'selected-btn':''}">${store.selected.has(s.id)?'✓ En mi agenda':'+ Añadir a mi agenda'}</button><button id="favSession">${store.fav.has(s.id)?'★ Favorita':'☆ Favorita'}</button><button id="copyTitle">Copiar título</button></div><div class="source-note">Descripciones resumidas a partir de la agenda del organizador. La recomendación y el ranking son personalizados, no oficiales.</div></div>`;
